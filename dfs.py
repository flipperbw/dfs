...
mycursor.execute('delete from optimization where week = %s' % week)
			...
			if pulp.LpStatus[prob.status] != 'Optimal':
				...
				for v in prob.variables():
					if v.varValue == 1:
						...
						mycursor.execute('insert into optimization values("%s", "%s", "%s", "%s", "%s", "%s");' % (old_name, k, week, cap, pt, current_time))
